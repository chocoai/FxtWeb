...
class ExcelDiff(object):
    ...
    def diff_file(self):
        ...
        for i in range(len(sheetn)):
            diffRP=self.diff_sheet(wb1.sheet_by_index(i), wb2.sheet_by_index(i))
            if diffRP:
                #print diffRP
    ...
    def diff_sheet(self,sheet1, sheet2):
        """
        diff两个Sheet
        :param sheet1:
        :param sheet2:
        ...
        for r in range(nr):
            ...
            if row1 == None and row2 != None:
                diff = 2
                report.append("+ROW[" + str(r + 1) + "]: " + self.row_to_str(row2))

            if row1 == None and row2 == None:
                diff = 0
            if row1 != None and row2 == None:
                diff = 3
                report.append("-ROW[" + str(r + 1) + "]: " + self.row_to_str(row1))
            if row1 != None and row2 != None:
                # diff the two rows
                reportRow = self.diff_row(row1, row2)
                if len(reportRow) > 0:
                    report.append("#ROW[" + str(r + 1) + "]1: " + self.row_to_str(row1))
                    report.append("#ROW[" + str(r + 1) + "]2: " + self.row_to_str(row2))
